...
int main(void) {
    ...
    USART1Pointer = initBufferedUSART(USART1, 50, 50);

    char buffer[50] = {0};

    while (1) {

        if (isRxBufferNotEmptyUSART(USART1Pointer)) {
            readStringUSART(USART1Pointer, buffer);
            sendFormattedStringUSART(USART1Pointer, 50, "ECHO %s\r\n", buffer);
            memset(buffer, 0, 50);
        }
